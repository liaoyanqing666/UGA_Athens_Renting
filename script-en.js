...
const noteLabels = {
  "Min Price Per Person": "Min Price Per Person",
  Size: "Size",
  "Drive to Boyd (mile/min)": "Drive to Boyd",
...
const regionPriorityRules = [
  ...
  { test: (value) => value.includes("campus"), rank: 1 },
  { test: (value) => value.includes("athens"), rank: 2 },
  { test: (value) => value.includes("nearby"), rank: 3 },
];

init().catch((error) => {
  console.error(error);
  elements.cards.innerHTML = '<div class="empty-state">Failed to load data. Make sure <code>data/apartments_EN.json</code> has been generated.</div>';
});
...
function populateLayoutOptions(apartments) {
  ...
  const uniqueLayouts = [...new Set(layouts)].sort((a, b) => a.localeCompare(b, "en"));
...
function applyFilters() {
  ...
  state.filtered = state.apartments.filter((apartment) => {
    ...
    if (furnished && sanitizeText(apartment.furnished) !== furnished) {
      return false;
    }
...
function sortApartments() {
  const sorters = {
    "region-asc": (a, b) => compareRegion(a.region, b.region) || a.name.localeCompare(b.name, "en"),
    "price-asc": (a, b) => compareNullable(a.summary.minPricePerPerson, b.summary.minPricePerPerson) || a.name.localeCompare(b.name, "en"),
    "rating-desc": (a, b) => compareNullable(b.googleRating, a.googleRating) || a.name.localeCompare(b.name, "en"),
    "drive-asc": (a, b) => compareNullable(a.commute.drive.minutes, b.commute.drive.minutes) || a.name.localeCompare(b.name, "en"),
    "bus-asc": (a, b) => compareNullable(a.commute.bus.minutes, b.commute.bus.minutes) || a.name.localeCompare(b.name, "en"),
...
function render() {
  ...
  elements.resultsMeta.textContent = `Showing ${state.filtered.length} / ${state.apartments.length} apartments`;

  if (!state.filtered.length) {
    elements.cards.innerHTML = '<div class="empty-state">No apartments match the current filters.</div>';
    return;
  }
...
function fillCardMetrics(card, apartment) {
  ...
  card.querySelector(".metric__rating").textContent = apartment.googleRating ? apartment.googleRating.toFixed(1) : "Unknown";
...
function fillCardDetails(card, apartment) {
  card.querySelector(".card__reviews").textContent = sanitizeText(apartment.googleReviewSummary) || "No review summary";
  card.querySelector(".card__fees").textContent = sanitizeText(apartment.fees) || "No fee details";
  card.querySelector(".card__location").textContent = sanitizeText(apartment.location) || "N/A";
  card.querySelector(".card__amenities").textContent = sanitizeText(apartment.amenitiesText) || "N/A";
  card.querySelector(".card__notes").textContent = sanitizeText(apartment.notes) || "N/A";
  card.querySelector(".card__raw-commute").textContent = [
    `Drive ${sanitizeText(apartment.raw?.drive) || "Unknown"}`,
    `Bike ${sanitizeText(apartment.raw?.bike) || "Unknown"}`,
    `Bus ${sanitizeText(apartment.raw?.bus) || "Unknown"}`,
  ].join(" / ");
}
...
function fillCardTags(card, apartment) {
  ...
  createTag(tags, sanitizeText(apartment.furnished) || "Furnishing unknown");

  if (apartment.commute.bus.hasShuttle === true) {
    createTag(tags, "Shuttle");
  }
...
function compareRegion(a, b) {
  ...
  return sanitizeText(a).localeCompare(sanitizeText(b), "en");
}

function getRegionRank(region) {
  const value = sanitizeText(region).toLowerCase();
...
function formatNullable(value, suffix = "") {
  return value === null || value === undefined ? "Unknown" : `${Math.round(value)}${suffix}`;
}
